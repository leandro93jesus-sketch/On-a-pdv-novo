...
CREATE TABLE IF NOT EXISTS sale_events(
 id TEXT PRIMARY KEY, sale_id TEXT NOT NULL, event_type TEXT NOT NULL, operator_id TEXT NOT NULL, reason TEXT NOT NULL, details TEXT, created_at TEXT NOT NULL,
 FOREIGN KEY(sale_id) REFERENCES sales(id));
CREATE INDEX IF NOT EXISTS ix_sale_events_sale ON sale_events(sale_id,created_at);
CREATE TABLE IF NOT EXISTS credit_receipt_reversals(
 id TEXT PRIMARY KEY, receipt_id TEXT NOT NULL UNIQUE, account_id TEXT NOT NULL, amount NUMERIC NOT NULL, operator_id TEXT NOT NULL, reason TEXT NOT NULL, created_at TEXT NOT NULL,
 FOREIGN KEY(receipt_id) REFERENCES credit_receipts(id), FOREIGN KEY(account_id) REFERENCES credit_accounts(id));
CREATE TABLE IF NOT EXISTS quotes(
 id TEXT PRIMARY KEY, number INTEGER NOT NULL UNIQUE, created_at TEXT NOT NULL, customer_id TEXT, discount NUMERIC NOT NULL, total NUMERIC NOT NULL, status TEXT NOT NULL DEFAULT 'Draft', valid_until TEXT, notes TEXT,
 FOREIGN KEY(customer_id) REFERENCES customers(id));
CREATE TABLE IF NOT EXISTS quote_items(
 id TEXT PRIMARY KEY, quote_id TEXT NOT NULL, product_id TEXT NOT NULL, code TEXT NOT NULL, name TEXT NOT NULL, quantity NUMERIC NOT NULL, unit_price NUMERIC NOT NULL, subtotal NUMERIC NOT NULL,
 FOREIGN KEY(quote_id) REFERENCES quotes(id));
CREATE INDEX IF NOT EXISTS ix_quotes_created ON quotes(created_at);
CREATE TABLE IF NOT EXISTS fiscal_drafts(
 id TEXT PRIMARY KEY, sale_id TEXT NOT NULL UNIQUE, created_at TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'Draft', notes TEXT, updated_at TEXT NOT NULL,
 FOREIGN KEY(sale_id) REFERENCES sales(id));
CREATE INDEX IF NOT EXISTS ix_fiscal_drafts_status ON fiscal_drafts(status,created_at);
CREATE TABLE IF NOT EXISTS cash_reopen_events(
 id TEXT PRIMARY KEY, session_id TEXT NOT NULL, operator_id TEXT NOT NULL, reason TEXT NOT NULL, created_at TEXT NOT NULL, previous_closed_at TEXT, previous_informed_total NUMERIC,
 FOREIGN KEY(session_id) REFERENCES cash_sessions(id));
CREATE INDEX IF NOT EXISTS ix_cash_movements_created ON cash_movements(created_at);
...
public sealed class SqliteCashSessionRepository(OncaDatabase database,IClock clock):ICashSessionRepository
{
    ...
    {
        await using var c=database.Open();await using(var find=c.CreateCommand()){find.CommandText="SELECT id,opened_at,opening_amount FROM cash_sessions WHERE operator_id=$op AND closed_at IS NULL ORDER BY opened_at DESC LIMIT 1";find.Parameters.AddWithValue("$op",operatorId.ToString());await using var r=await find.ExecuteReaderAsync(ct);if(await r.ReadAsync(ct))return new(Guid.Parse(r.GetString(0)),operatorId,DateTimeOffset.Parse(r.GetString(1)),r.GetDecimal(2));}
        var session=new CashSession(Guid.NewGuid(),operatorId,clock.Now,openingAmount);await using var cmd=c.CreateCommand();cmd.CommandText="INSERT INTO cash_sessions(id,operator_id,opened_at,opening_amount) VALUES($id,$op,$at,$amount)";cmd.Parameters.AddWithValue("$id",session.Id.ToString());cmd.Parameters.AddWithValue("$op",operatorId.ToString());cmd.Parameters.AddWithValue("$at",session.OpenedAt.ToString("O"));cmd.Parameters.AddWithValue("$amount",openingAmount);await cmd.ExecuteNonQueryAsync(ct);return session;
    }
}
