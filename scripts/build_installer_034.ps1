...
$x64=Join-Path $root 'portable032\ONCA-PDV-PRO-0.1.32-AGUARDO-CANCELAR-ESTOQUE-TESTADO.exe'
$x86=Join-Path $root 'portable034x86\ONCA-PDV-PRO-x86.exe'
...
$out=(Resolve-Path (Join-Path $root 'portable032')).Path
$iss=@"
[Setup]
AppId=ONCA-PDV-PRO
...
OutputDir=$out
OutputBaseFilename=ONCA-PDV-PRO-Setup-0.1.34
Compression=lzma2
...
$issPath=Join-Path $out 'onca-pdv-034.iss'
Set-Content -Path $issPath -Value $iss -Encoding UTF8
& $iscc $issPath
if($LASTEXITCODE -ne 0){throw "ISCC failed with code $LASTEXITCODE"}
$setup=Join-Path $out 'ONCA-PDV-PRO-Setup-0.1.34.exe'
