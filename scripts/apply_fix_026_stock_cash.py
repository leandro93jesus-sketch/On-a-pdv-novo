...
# --- 0.1.30 improved inventory: clear dashboard/actions, preserving existing services and data. ---
p=d/'InventoryWindow.xaml'; p.write_text(r'''<Window x:Class="OncaPDV.Desktop.InventoryWindow" xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml" Title="ESTOQUE — ONÇA PDV PRO 0.1.30" Width="1280" Height="820" MinWidth="1040" MinHeight="680" WindowStartupLocation="CenterOwner" Background="#F4F7F5">
<Grid Margin="20"><Grid.RowDefinitions><RowDefinition Height="Auto"/><RowDefinition Height="Auto"/><RowDefinition Height="Auto"/><RowDefinition/><RowDefinition Height="Auto"/><RowDefinition Height="170"/></Grid.RowDefinitions>
<Grid><StackPanel><TextBlock Text="ESTOQUE" FontSize="30" FontWeight="Bold" Foreground="#0B6B3A"/><TextBlock Text="Cadastro, preços, código de barras e movimentações em uma única tela." Foreground="#66746C"/></StackPanel><Button HorizontalAlignment="Right" Content="+ CADASTRAR PRODUTO" Padding="20,12" Background="#0B6B3A" Foreground="White" FontWeight="Bold" Click="NewProduct_Click"/></Grid>
<UniformGrid Grid.Row="1" Columns="4" Margin="0,14,0,8"><Border Background="White" CornerRadius="10" Padding="14" Margin="4"><StackPanel><TextBlock Text="PRODUTOS" Foreground="#66746C"/><TextBlock x:Name="TotalProductsText" Text="0" FontSize="24" FontWeight="Bold"/></StackPanel></Border><Border Background="White" CornerRadius="10" Padding="14" Margin="4"><StackPanel><TextBlock Text="ATIVOS" Foreground="#66746C"/><TextBlock x:Name="ActiveProductsText" Text="0" FontSize="24" FontWeight="Bold" Foreground="#0B6B3A"/></StackPanel></Border><Border Background="White" CornerRadius="10" Padding="14" Margin="4"><StackPanel><TextBlock Text="DESATIVADOS" Foreground="#66746C"/><TextBlock x:Name="InactiveProductsText" Text="0" FontSize="24" FontWeight="Bold"/></StackPanel></Border><Border Background="White" CornerRadius="10" Padding="14" Margin="4"><StackPanel><TextBlock Text="ESTOQUE BAIXO / ZERADO" Foreground="#66746C"/><TextBlock x:Name="LowStockText" Text="0" FontSize="24" FontWeight="Bold" Foreground="#A63A32"/></StackPanel></Border></UniformGrid>
<Grid Grid.Row="2" Margin="4,6,4,12"><Grid.ColumnDefinitions><ColumnDefinition/><ColumnDefinition Width="Auto"/></Grid.ColumnDefinitions><TextBox x:Name="SearchBox" FontSize="16" Padding="11" KeyDown="SearchBox_KeyDown" ToolTip="Nome, código ou código de barras"/><Button Grid.Column="1" Content="BUSCAR" Padding="24,11" Margin="8,0,0,0" Background="#173A28" Foreground="White" Click="Search_Click"/></Grid>
<DataGrid Grid.Row="3" x:Name="Grid" AutoGenerateColumns="False" IsReadOnly="True" SelectionMode="Single" SelectionChanged="Grid_SelectionChanged" RowHeight="40" Background="White" GridLinesVisibility="Horizontal"><DataGrid.Columns><DataGridTextColumn Header="CÓDIGO" Binding="{Binding InternalCode}" Width="120"/><DataGridTextColumn Header="CÓD. BARRAS" Binding="{Binding Barcode}" Width="170"/><DataGridTextColumn Header="PRODUTO" Binding="{Binding Name}" Width="2*"/><DataGridTextColumn Header="UN." Binding="{Binding Unit}" Width="70"/><DataGridTextColumn Header="QUANTIDADE" Binding="{Binding Stock,StringFormat=N0}" Width="115"/><DataGridTextColumn Header="PREÇO VENDA" Binding="{Binding SalePrice,StringFormat=C}" Width="125"/><DataGridCheckBoxColumn Header="ATIVO" Binding="{Binding Active}" Width="75"/></DataGrid.Columns></DataGrid>
<Grid Grid.Row="4" Margin="0,10"><Grid.ColumnDefinitions><ColumnDefinition/><ColumnDefinition Width="Auto"/></Grid.ColumnDefinitions><TextBlock x:Name="SelectedText" VerticalAlignment="Center" FontSize="16" FontWeight="SemiBold" Text="Selecione um produto."/><WrapPanel Grid.Column="1"><Button Content="EDITAR / ALTERAR PREÇO" Padding="13,10" Margin="3" Click="Edit_Click"/><Button Content="+ ENTRADA" Padding="13,10" Margin="3" Background="#0B6B3A" Foreground="White" Click="Entry_Click"/><Button Content="- SAÍDA" Padding="13,10" Margin="3" Background="#A63A32" Foreground="White" Click="Exit_Click"/><Button Content="AJUSTAR QUANTIDADE" Padding="13,10" Margin="3" Click="Balance_Click"/></WrapPanel></Grid>
<GroupBox Grid.Row="5" Header="HISTÓRICO DE MOVIMENTAÇÕES"><DataGrid x:Name="HistoryGrid" AutoGenerateColumns="False" IsReadOnly="True" Background="White"><DataGrid.Columns><DataGridTextColumn Header="DATA" Binding="{Binding CreatedAt,StringFormat=dd/MM/yyyy HH:mm}" Width="160"/><DataGridTextColumn Header="TIPO" Binding="{Binding Type}" Width="130"/><DataGridTextColumn Header="MOVIMENTO" Binding="{Binding Quantity,StringFormat=N3}" Width="120"/><DataGridTextColumn Header="MOTIVO" Binding="{Binding Reason}" Width="*"/></DataGrid.Columns></DataGrid></GroupBox>
</Grid></Window>''',encoding='utf-8')

p=d/'InventoryWindow.xaml.cs'; s=p.read_text(encoding='utf-8-sig')
s=s.replace('private async Task Refresh(){Grid.ItemsSource=await _svc.SearchAsync(SearchBox.Text.Trim());}', '''private async Task Refresh(){var products=await _svc.SearchAsync(SearchBox.Text.Trim());Grid.ItemsSource=products;TotalProductsText.Text=products.Count.ToString();ActiveProductsText.Text=products.Count(x=>x.Active).ToString();InactiveProductsText.Text=products.Count(x=>!x.Active).ToString();LowStockText.Text=products.Count(x=>x.Stock<=5).ToString();}''')
s=s.replace('catch(Exception ex){MessageBox.Show(ex.Message,"ESTOQUE",MessageBoxButton.OK,MessageBoxImage.Warning);}}\\n    private async void Edit_Click', 'catch(Exception ex){ShowProductError(ex);}}\\n    private async void Edit_Click')
s=s.replace('catch(Exception ex){MessageBox.Show(ex.Message,"Produtos / Estoque",MessageBoxButton.OK,MessageBoxImage.Warning);}}\\n    private async void Entry_Click', 'catch(Exception ex){ShowProductError(ex);}}\\n    private async void Entry_Click')
anchor='    private Product? Need()'
if anchor not in s: raise RuntimeError('Need anchor missing')
s=s.replace(anchor,'''    private static void ShowProductError(Exception ex){var m=ex.Message??"";if(m.Contains("UNIQUE",StringComparison.OrdinalIgnoreCase)||m.Contains("barcode",StringComparison.OrdinalIgnoreCase)||m.Contains("BARRAS",StringComparison.OrdinalIgnoreCase))m="CÓDIGO OU CÓDIGO DE BARRAS DUPLICADO. INFORME UM CÓDIGO ÚNICO.\\n\\n"+m;MessageBox.Show(m,"ESTOQUE",MessageBoxButton.OK,MessageBoxImage.Warning);}
    '''+anchor,1)
p.write_text(s,encoding='utf-8')
