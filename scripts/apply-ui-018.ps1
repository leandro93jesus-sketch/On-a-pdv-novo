...
$root=(Resolve-Path '.\work-final\ONCA-PDV-PRO').Path
$desktop=Join-Path $root 'src\OncaPDV.Desktop'
...
$needle='    protected override void OnStartup(StartupEventArgs e)\n    {'
if(-not $t.Contains('DefaultThreadCurrentCulture')){
  if(-not $t.Contains($needle)){throw 'App OnStartup anchor missing'}
  $inject=@'
...
'@ | Set-Content (Join-Path $desktop 'DiversosWindow.xaml') -Encoding UTF8

# Dedicated Backup window.
@'
<Window x:Class="OncaPDV.Desktop.BackupWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
'@ | Set-Content (Join-Path $desktop 'BackupWindow.xaml') -Encoding UTF8
...
'@ | Set-Content (Join-Path $desktop 'BackupWindow.xaml.cs') -Encoding UTF8

# Window size settings.
@'
<Window x:Class="OncaPDV.Desktop.DisplaySettingsWindow" xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
'@ | Set-Content (Join-Path $desktop 'DisplaySettingsWindow.xaml') -Encoding UTF8
...
public partial class DisplaySettingsWindow : Window
{
    ...
    private void SetSize(double w, double h) { _target.WindowState = WindowState.Normal; _target.Width = Math.Max(1240, w); _target.Height = Math.Max(760, h); _target.Left = Math.Max(0, (SystemParameters.WorkArea.Width - _target.Width) / 2); _target.Top = Math.Max(0, (SystemParameters.WorkArea.Height - _target.Height) / 2); }
...
'@ | Set-Content (Join-Path $desktop 'DisplaySettingsWindow.xaml.cs') -Encoding UTF8

# Add dedicated navigation entries and remove the confusing Backup label from the Caixa utility button.
$mainXaml=Join-Path $desktop 'MainWindow.xaml'
$x=Get-Content $mainXaml -Raw
$x=$x.Replace('Content="Caixa / Backup"','Content="Caixa"')
if(-not $x.Contains('Click="BackupCenter_Click"')){
  $anchor='<Button Style="{StaticResource NavButton}" Content="⚙   Configurações" Click="Printer_Click"/>'
  if(-not $x.Contains($anchor)){throw 'MainWindow settings nav anchor missing'}
...
Set-Content $mainXaml $x -Encoding UTF8

$mainCs=Join-Path $desktop 'MainWindow.xaml.cs'
$c=Get-Content $mainCs -Raw
if(-not $c.Contains('BackupCenter_Click(object sender')){
  $anchor='    private void Printer_Click(object sender, RoutedEventArgs e) => new PrinterSettingsWindow(_paths) { Owner = this }.ShowDialog();'
  if(-not $c.Contains($anchor)){throw 'MainWindow Printer_Click anchor missing'}
  $extra=$anchor+"`r`n    private void BackupCenter_Click(object sender, RoutedEventArgs e) => new BackupWindow(_database, _paths) { Owner = this }.ShowDialog();`r`n    private void DisplaySettings_Click(object sender, RoutedEventArgs e) => new DisplaySettingsWindow(this) { Owner = this }.ShowDialog();"
  $c=$c.Replace($anchor,$extra)
}
Set-Content $mainCs $c -Encoding UTF8

Write-Host 'UI018_APPLIED=YES'
