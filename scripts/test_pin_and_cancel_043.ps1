...
$exe=(Resolve-Path '.\portable043\ONCA-PDV-PRO-0.1.43-PIN-RECUPERADO.exe').Path
...
try {
 ...
 if($LASTEXITCODE -ne 0){throw "Prior version PIN seed failed"}
 ...
 if(@(Windows $p|Where-Object {$_.Title -like '*RECUPERAR PIN*'}).Count -gt 0){throw "Recovery screen reopened after successful one-time reset"}
 Write-Host "RECOVERY_SCREEN_NOT_REPEATED_ON_RESTART=YES"
 $mainRoot=Root $main
 ...
 Write-Host "UI_CANCEL_043_FULL_FLOW_PASS=YES"
 if($env:GITHUB_ENV){Add-Content -Path $env:GITHUB_ENV -Value "ONCA_PDV_DATA_ROOT=$env:ONCA_PDV_DATA_ROOT"}
}
finally {Try-Close $p}
